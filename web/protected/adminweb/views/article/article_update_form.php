...
<tr>
 ...
 <tr>
    <td class="tb_title">组图：</td><td><span id="notebookMessage"></span></td>
  </tr>
  <tr >
    <td><div>
...
function uploadifyRemove(fileId,attrName){
  //if(confirm('本操作不可恢复，确定继续？')){
    ...
    $("#notebookMessage").html('<span style="color:#FF0000"><img src="<?php echo Yii::app()->request->baseUrl;?>/images/adminweb/loading.gif" align="absmiddle">正在更新数据...</span>'); 
    $.post("?r=uploadify/remove",{imageId:fileId},function(res){
        $("#"+attrName+fileId).remove();
        $("#notebookMessage").html('<span style="color:#FF0000">'+res.message+'</span>'); 
